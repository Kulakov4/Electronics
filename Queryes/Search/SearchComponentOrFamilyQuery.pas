...
type
  ...
  public
    function SearchComponentWithProducer(const AComponentName, AProducer: string): Integer;
        overload;
    function SearchComponentWithProducer(const AComponentName: string): Integer;
        overload;
    ...
    function SearchComponen(AParentID: Integer;
...
function TQuerySearchComponentOrFamily.SearchComponentWithProducer(const AComponentName,
...
function TQuerySearchComponentOrFamily.SearchByValue(const AComponentName: string):
    Integer;
var
  ACondition: string;
begin
...
function TQuerySearchComponentOrFamily.SearchComponen(AParentID: Integer;
...
begin
  ...
  // Добавляем условие
  ACondition := 'upper(p.Value) = upper(:Value) and ParentProductID = :ParentProductID';
  FDQuery.SQL.Text := Replace(fdqBase.SQL.Text, ACondition, '0=0');
  SetParamType('Value', ptInput, ftString);
